...
class Model {
      ...
      if (!this.Schema.definition[field]) {
        ...
        switch (schemaField.type) {
          ...
          case Array:
            ...
            break;
          case Mixed:
            if ((typeof sanitizedData[field] || typeof data[field]) !== true) {
              errors.push(`"${field}" must be a mixed`);
            }
            break;
          case uuid:
            if (!(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(typeof sanitizedData[field] || typeof data[field]))) {
  ...
  async aggregate(pipeline, data) {
    ...
    for (const stage of pipeline) {
      ...
      const operation22 = stage.$unwind ? Object.keys(stage)[0] : null;
      switch (operation || operation22 || operations || operation11 || operation1 || operations1 || operation2 || operations2) {
        // $match operation
        case '$match':
        ...
        case '$unwind':
          currentData = currentData.reduce((acc, item) => {
            const fieldToUnwind = stage.$unwind;
            if (Array.isArray(item[fieldToUnwind])) {
              acc.push(...item[fieldToUnwind].map((unwoundItem) => ({ ...item, [fieldToUnwind]: unwoundItem })));
            } else {
              acc.push(item);
            }
            return acc;
